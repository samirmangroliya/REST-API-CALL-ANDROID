...
import com.zeustechnocrats.salesmantracking.global.GlobalConstant;
import com.zeustechnocrats.salesmantracking.global.Utils;
...
public class ParseController {
    ...
    class AsyncHttpRequest extends AsyncTask<Void, Void, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            if (activity != null && activity instanceof Activity && !((Activity) activity).isFinishing() && isShowLoading) {
    ...
    public static String fileVideoAndImageUploading(Map<String, String> hashMap, int k) {
        try {
            ...
            if (hashMap.containsKey("image1")) {
                String strFilePath1 = hashMap.get("image1");
                hashMap.remove("image1");
                File file1 = new File(strFilePath1);

                if (file1 != null && file1.exists()) {
                    multipartBody.addFormDataPart("image1", file1.getName(), RequestBody.create(MEDIA_TYPE_PNG, file1));
                } else {
                    Log.d("file1 path error", "path1 not found: " + strFilePath1);
                }


                if (hashMap.containsKey("image2")) {
                    String strFilePath2 = hashMap.get("image2");
                    hashMap.remove("image2");
                    File file2 = new File(strFilePath2);

                    if (file2 != null && file2.exists()) {
                        multipartBody.addFormDataPart("image2", file2.getName(), RequestBody.create(MEDIA_TYPE_PNG, file2));
                    } else {
                        Log.d("file2 path error", "path2 not found: " + strFilePath2);
                    }

                }
                if (hashMap.containsKey("image3")) {
                    String strFilePath3 = hashMap.get("image3");
                    hashMap.remove("image3");
                    File file3 = new File(strFilePath3);

                    if (file3 != null && file3.exists()) {
                        multipartBody.addFormDataPart("image3", file3.getName(), RequestBody.create(MEDIA_TYPE_PNG, file3));
                    } else {
                        Log.d("file3 path error", "path3 not found: " + strFilePath3);
                    }

                }

                if (hashMap.containsKey("image4")) {
                    String strFilePath4 = hashMap.get("image4");
                    hashMap.remove("image4");
                    File file4 = new File(strFilePath4);

                    if (file4 != null && file4.exists()) {
                        multipartBody.addFormDataPart("image4", file4.getName(), RequestBody.create(MEDIA_TYPE_PNG, file4));
                    } else {
                        Log.d("file4 path error", "path4 not found: " + strFilePath4);
                    }
    ...
    public void checkResponse(String response) {
        ...
        } else if (response.equals(strInternet)) {
            listener.onFailed(statusCode, strInternet);
        } else {
